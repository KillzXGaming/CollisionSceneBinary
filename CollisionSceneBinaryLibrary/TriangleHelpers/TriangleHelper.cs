...
namespace CollisionSceneBinaryTool
{
    public class TriangleHelper
    {
		...
		public static bool TriangleCubeOverlap(Triangle t, Vector3 Position, float BoxSize)
		{
			...
			float boxHalfSize = BoxSize;

			//Ignore height for now
			//Octrees just divide by width/depth
			float box_height = 10000;

            // Get the AABB bounds
            Vector3 boxMin = boxCenter - new Vector3(boxHalfSize, box_height, boxHalfSize);
            Vector3 boxMax = boxCenter + new Vector3(boxHalfSize, box_height, boxHalfSize);

            // Test each axis of the box
            if (Math.Min(triangle.Vertices[0].X, Math.Min(triangle.Vertices[1].X, triangle.Vertices[2].X)) > boxMax.X) return false;
            if (Math.Max(triangle.Vertices[0].X, Math.Max(triangle.Vertices[1].X, triangle.Vertices[2].X)) < boxMin.X) return false;
            if (Math.Min(triangle.Vertices[0].Y, Math.Min(triangle.Vertices[1].Y, triangle.Vertices[2].Y)) > boxMax.Y) return false;
            if (Math.Max(triangle.Vertices[0].Y, Math.Max(triangle.Vertices[1].Y, triangle.Vertices[2].Y)) < boxMin.Y) return false;
            if (Math.Min(triangle.Vertices[0].Z, Math.Min(triangle.Vertices[1].Z, triangle.Vertices[2].Z)) > boxMax.Z) return false;
            if (Math.Max(triangle.Vertices[0].Z, Math.Max(triangle.Vertices[1].Z, triangle.Vertices[2].Z)) < boxMin.Z) return false;
